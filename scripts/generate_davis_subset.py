...
class DavisTest(Dataset):
    def __init__(self, img_size, data_root, split):
        ...
        if split == 'trainval':
            all_folders = glob.glob(os.path.join(self.data_root, 'davis2017/DAVIS-2017-trainval-480p', 'DAVIS/JPEGImages/480p', '*'))
        elif split == 'test':
            all_folders = glob.glob(os.path.join(self.data_root, 'davis2017/DAVIS-2017-test-dev-480p', 'DAVIS/JPEGImages/480p', '*'))
...
if __name__ == '__main__':
    ...
    loader = get_loader('trainval', img_size=512, data_root=data_root, batch_size=1, shuffle=False, num_workers=4)
    ...
    filtering_vis(data_root, 'davis', 'trainval', flow_data_path, threshold_dict, mse_threshold)

    flow_data_path = os.path.join(data_root, 'flow_data/davis_test.json')
    filtering_vis(data_root, 'davis', 'test', flow_data_path, threshold_dict, mse_threshold)