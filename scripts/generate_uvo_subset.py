...
def get_folders(data_root, json_path):
    with open(json_path, 'r') as f:
        data = json.load(f)
    return [os.path.join(data_root, '{}'.format(item['ytid'])) for item in data['videos']]
...
class UVOTest(Dataset):
    def __init__(self, img_size, data_root, split, sampling=1):
        ...
        if split == 'train':
            json_path = os.path.join(data_root, 'uvo_frames/UVO_video_{}_dense.json'.format(split))
        elif split == 'test':
            json_path = os.path.join(data_root, 'uvo_frames/UVO_video_{}_dense.json'.format(split))
        all_folders = get_folders(os.path.join(data_root, 'uvo_frames/uvo_videos_dense'), json_path)
...
if __name__ == '__main__':
    ...
    sampling = 1
    loader = get_loader(sampling, 'test', img_size=512, data_root=data_root, batch_size=1, shuffle=False, num_workers=4)
    ...
    filtering_vis(data_root, 'uvo', 'train_n1', flow_data_path, threshold_dict, mse_threshold)

    flow_data_path = os.path.join(data_root, 'flow_data/uvo_train_n5.json')
    filtering_vis(data_root, 'uvo', 'train_n5', flow_data_path, threshold_dict, mse_threshold)

    flow_data_path = os.path.join(data_root, 'flow_data/uvo_test_n1.json')
    filtering_vis(data_root, 'uvo', 'test_n1', flow_data_path, threshold_dict, mse_threshold)

    flow_data_path = os.path.join(data_root, 'flow_data/uvo_test_n5.json')
    filtering_vis(data_root, 'uvo', 'test_n5', flow_data_path, threshold_dict, mse_threshold)