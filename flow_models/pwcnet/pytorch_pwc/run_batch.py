...
for strOption, strArgument in getopt.getopt(sys.argv[1:], '', [ strParameter[2:] + '=' for strParameter in sys.argv[1::2] ])[0]:
  ...
  if strOption == '--pairs' and strArgument != '': arguments_strModel = strArgument # which model to use
...
if __name__ == '__main__':
  tenFirst = torch.FloatTensor(numpy.ascontiguousarray(numpy.array(PIL.Image.open(file1))[:, :, ::-1].transpose(2, 0, 1).astype(numpy.float32) * (1.0 / 255.0)))
  tenSecond = torch.FloatTensor(numpy.ascontiguousarray(numpy.array(PIL.Image.open(file2))[:, :, ::-1].transpose(2, 0, 1).astype(numpy.float32) * (1.0 / 255.0)))
