...
import numpy as np
try:
  from flow_models.pwcnet.pytorch_pwc.correlation import correlation # the custom cost volume layer
except:
  sys.path.insert(0, 'flow_models/pwcnet/pytorch_pwc/correlation'); import correlation # you should consider upgrading python
# end
...
def run_flow(im_file1, im_file2):
  tenFirst = torch.FloatTensor(numpy.ascontiguousarray(numpy.array(PIL.Image.open(im_file1))[:, :, ::-1].transpose(2, 0, 1).astype(numpy.float32) * (1.0 / 255.0)))
  tenSecond = torch.FloatTensor(numpy.ascontiguousarray(numpy.array(PIL.Image.open(im_file2))[:, :, ::-1].transpose(2, 0, 1).astype(numpy.float32) * (1.0 / 255.0)))
