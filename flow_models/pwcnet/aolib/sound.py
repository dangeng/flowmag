import wave, aolib.util as ut, os, numpy as np, aolib.img as ig, aolib.imtable as imtable, pylab as pl, copy
...
def vis_specgram_at_times(snd, times, window_sec, par = 0, im_shape = (500, 888), compress_ims = False):
  ...
  if not compress_ims:
    ims = list(map(ig.uncompress, ip.map(par, f, times)))
  return ims
...
class Sound:
  ...
  def __getitem__(self, *args):
    return Sound(self.times.__getitem__(*args), self.rate,
                 self.samples.__getitem__(*args))
